...
Snow::Snow()
{
    ...
    mUpdateShader = new Shader("./shaders/SnowUpdate.vs",
                               "./shaders/SnowUpdate.fs",
                               "./shaders/SnowUpdate.gs", varyings, 5);
    //设置TransformFeedback缓存能够记录的顶点的数据类型
    
    mRenderShader = new Shader("./shaders/SnowRender.vs",
                               "./shaders/SnowRender.fs");
    mRenderShadow = new Shader("./shaders/SnowShadow.vs",
                               "./shaders/SnowShadow.fs");
    //设置随机纹理
    InitRandomTexture(512);
    mSparkTexture.loadTexture("./assets/images/rain4.bmp",GL_TRUE);
    mRenderShader->use();
