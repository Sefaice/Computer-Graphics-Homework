...
Camera camera(glm::vec3(0.0, 5.0, 10.0), glm::vec3(0.0, 1.0, 0.0));
...
void drawModels(){
    ...
    //bamboo
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(30.0f, -100.0f, 200.0f)); // translate it down so it's at the center of the scene
    ...
    //bamboo2
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(60.0f, -100.0f, 220.0f)); // translate it down so it's at the center of the scene
    ...
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(90.0f, -100.0f, 220.0f)); // translate it down so it's at the center of the scene
    ...
    //house
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(100.0f, -100.0f, 180.0f)); // translate it down so it's at the center of the scene
    model = glm::scale(model, glm::vec3(10.0f, 10.0f, 10.0f));    // it's a bit too big for our scene, so scale it down
    model_shader.SetMatrix4("model", model);
